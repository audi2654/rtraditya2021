//C Pointers assignment : 2D Arrays	- C
...
int main(int argc, char* argv[], char* envp[])
{
	...
	{
		amp_iArray[i] = (int *)malloc((NUM_COLUMNS - i) * sizeof(int));
		//row 0 will have (NUM_COLUMNS - 0) = 5-0 = 5 columns
		//row 1 will have (NUM_COLUMNS - 1) = 5-1 = 3 columns
		//row 2 will have (NUM_COLUMNS - 2) = 5-2 = 2 columns
		//and likewise
		//because of this, there is no contiguous memory allocation, hence although we may use the data as 2D array,
	...
	for(i = 0; i < NUM_ROWS; i++)
	{
		for(j = 0; j < (NUM_COLUMNS - i); j++)
		{
			printf("amp_iArray[%d][%i] = %u \t at addr : %p\n", i, j, amp_iArray[i][j], &amp_iArray[i][j]);
		}
		printf("\n");
	}
	printf("\n\n");
