...
void resize(int width, int height)
{
	...
	glMatrixMode(GL_PROJECTION);	//Perspective Projection Matrix, mhnje projection cha dabaa(stack) - OGL cha dusra dabaa
	glLoadIdentity();
...
void display(void)
{
	...
	//try commenting & uncommenting below two lines & see output by quickly pressing F, 
	//the drawing comes very quickly & disappears WHY ?, also if glColor3f(1.0f, 1.0f, 1.0f); is removed from white tri & rect, they are drawn in blue color WHY ?
	//answer of both question is that OGL is a STATE MACHINE
	glMatrixMode(GL_MODELVIEW);		//Model-View Matrix, mhnje model view cha dabaa(stack) - OGL cha pahila dabaa
	glLoadIdentity();				//Identity Matrix - current CTM
	//above 2 lines tell CTM to comeback to center of screen after every game loop iteration, remember OGL is a state machine
	//after drawWhiteRectangle() CTM has changed its position & so when it goes back to game loop & comes back again
	//inside display() the CTM postion will be what it was set in previous state & it will start drawing vertices from that position which may go out of screen
	//so you bring back CTM to center of screen using above lines

	glTranslatef(0.0f, 0.0f, -9.0f); //new current CTM moved by -9.0 value on z axis multiplied with previous CTM
	drawColoredTriangle();

	glTranslatef(2.5f, 0.0f, 0.0f);	//new current CTM moved by 2.5 value on x axis multiplied with previous CTM
	drawWhiteTriangle();

	glTranslatef(-0.5f, 2.0f, 0.0f);	//new current CTM moved by 2.0 value on y axis multiplied with previous CTM
	drawWhiteRectangle();
