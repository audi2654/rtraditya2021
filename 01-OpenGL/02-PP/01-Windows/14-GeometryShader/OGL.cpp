//Date: 15/10/2022
//RTR2021 OGL Perspective Triangle Blue Screen with Shaders Template
//Program for Geometry Shader
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("AMP OGL PP - Triangle with Geometry Shader"),
...
int initialize(void)
{
	...
	//layout here means Geometry Domain
	...
	glCompileShader(geometryShaderObject);	//inline shader compiler

	//error checking for geometry shader
	status = 0;
	infoLogLength = 0;
	log = NULL;
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (log != NULL)
			{
				...
				fprintf(gpFile, "\nGeometry Shader Compilation Log: %s\n", log);
				free(log);
				uninitialize();
			}
		}
