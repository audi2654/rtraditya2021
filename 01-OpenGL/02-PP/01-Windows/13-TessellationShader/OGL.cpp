//Date: 14/10/2022
//RTR2021 OGL Perspective Triangle Blue Screen with Shaders
//Program for Tessellation Shader
...
GLuint mvpMatrixUniform;					//model view projection matrix user defined uniform type var

//user defined uniform vars for tessellation shader
...
int initialize(void)
{
	...
	glCompileShader(tessellationControlShaderObject);	//inline shader compiler

	//error checking for tessellation shader
	status = 0;
	infoLogLength = 0;
	log = NULL;
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (log != NULL)
			{
				...
				fprintf(gpFile, "\nTessellation Control Shader Compilation Log: %s\n", log);
				free(log);
				uninitialize();
			}
		}
	...
	glCompileShader(tessellationEvaluationShaderObject);	//inline shader compiler
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (log != NULL)
			{
				...
				fprintf(gpFile, "\nTessellation Evaluation Shader Compilation Log: %s\n", log);
				free(log);
				uninitialize();
			}
		}
	...
	glBindVertexArray(0);					//unbinding vertex array object for triangle
