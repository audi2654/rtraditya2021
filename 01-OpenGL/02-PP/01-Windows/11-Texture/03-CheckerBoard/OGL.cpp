//Date: 23/07/2022
//RTR2021 OGL PP Checkerboard Procedural/Mathematical Texture on 2D Geometry
...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(4 * 3 * sizeof(GLfloat)), NULL, GL_DYNAMIC_DRAW);						//putting position array in vbo in VRAM; GL_STATIC_DRAW means to fill buffer data now or on runtime, for runtime use GL_DYNAMIC_DRAW
...
void display(void)
{
	...
	glDrawArrays(GL_QUADS, 0, 4); 			//1 quad has 4 vertices
	glBindVertexArray(0);
...
void LoadGlTexture(void)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHECKERBOARD_WIDTH, CHECKERBOARD_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, gubaCheckerboard);
