//Date: 23/07/2022
//RTR2021 OGL PP Tweaked Smiley Texture on Quad
...
GLuint vbo_position;							//vertex buffer object for position
GLuint vbo_texcoord;							//vertex buffer object for texture

//for texture variables
GLuint texture_smiley;
GLint keyPressed = -1;

//for uniform attrib locations in shader
...
int initialize(void)
{
	...
	//vbo for position
	glGenBuffers(1, &vbo_position);																	//here vbo returns with a memory mapped I/O address from GPU VRAM, i.e watching GPU memory(khidki) from CPU memory(khidki)
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position);													//binding vbo to a target point of type of GL_ARRAY_BUFFER means, this vbo addr will hold array buffer data
	glBufferData(GL_ARRAY_BUFFER, sizeof(position), position, GL_STATIC_DRAW);						//putting position array in vbo in VRAM; GL_STATIC_DRAW means to fill buffer data now or on runtime, for runtime use GL_DYNAMIC_DRAW
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(4 * 2 * sizeof(GLfloat)), NULL, GL_DYNAMIC_DRAW);			//for texture array we had 4 lines of 2 elements
	...
	glBindVertexArray(0);					//unbinding vertex array object vao
...
void display(void)
{
	...
	modelViewMatrix = translationMatrix;		//here order of multiplication is very important & '*' is overloaded to multiply matrices in vmath.h
	...
	GLfloat texcoords[8];			//for different texcoords of smiley texture

	if(keyPressed == 1)				//full smiley face
	{
		texcoords[0] = {1.0f};
		texcoords[1] = {1.0f};
		texcoords[2] = {0.0f};
		...
		glUniform1i(keyPressedUniform, 1);			//sending uniform value for keyPressedUniform as 1 in shader for FragColor to work
	}
	else if(keyPressed == 2)		//half smile
	{
		texcoords[0] = {0.5f};
		texcoords[1] = {0.5f};
		texcoords[2] = {0.0f};
	...
	else if(keyPressed == 3)		//wallpaper tiling
	{
		texcoords[0] = {2.0f};
		texcoords[1] = {2.0f};
		texcoords[2] = {0.0f};
	...
	else if(keyPressed == 4)		//centre of smiley, nose dot pixel 
	{
		texcoords[0] = {0.5f};
		texcoords[1] = {0.5f};
		texcoords[2] = {0.5f};
	...
	{
		glUniform1i(keyPressedUniform, 0);			//sending uniform value for keyPressedUniform as 0 in shader for FragColor to work with only white color in else
	}
	...
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4); 			//1 quad has 4 vertices
	glBindVertexArray(0);

	glBindTexture(GL_TEXTURE_2D, 0);			//unbinding texture - optional
