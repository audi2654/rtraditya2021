//Date: 24/07/2022
//RTR2021 OGL PP Sphere with OBJ model loading with use of Texcoords
...
GLuint vbo_sphere_color;		//no use of color in this program
GLuint vbo_sphere_normal;
GLuint vbo_sphere_texture;
GLuint vbo_sphere_element;

//GLuint mvpMatrixUniform;		//splitting this in below 3 uniforms 
...
int initialize(void)
{
	...
	/* gl_Position = u_projectionMatrix * u_viewMatrix * u_modelMatrix * a_position;*/
	//this above order is very important -> projection, view, model, position
	...
	glBindAttribLocation(shaderProgramObject, AMP_ATTRIBUTE_COLOR, "a_color");					//no use of color in this program
	...
	getSphereVertexData(sphere_vertices, sphere_normals, sphere_textures, sphere_elements);		//all arrays go empty & come back filled 
	...
	//vbo for position
	glGenBuffers(1, &vbo_sphere_position);															//here vbo returns with a memory mapped I/O address from GPU VRAM, i.e watching GPU memory(khidki) from CPU memory(khidki)
	glBindBuffer(GL_ARRAY_BUFFER, vbo_sphere_position);												//binding vbo to a target point of type of GL_ARRAY_BUFFER means, this vbo addr will hold array buffer data
	glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_vertices), sphere_vertices, GL_STATIC_DRAW);		//putting position array in vbo in VRAM; GL_STATIC_DRAW means to fill buffer data now or on runtime, for runtime use GL_DYNAMIC_DRAW
	...
	//vbo for color
	glVertexAttrib3f(AMP_ATTRIBUTE_COLOR, 1.0f, 1.0f, 1.0f);										//no use of color in this program
	...
	glVertexAttribPointer(AMP_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 0, NULL);
	...
	glClearColor(0.5f, 0.5f, 0.5f, 1.0f);	//RGBA here we only tell which color to use when clearing screen, actual clearing happens in display()
	...
	//LoadGlTexture(&texture_sphere, MAKEINTRESOURCE(IDBITMAP_SPHERE)) == FALSE);
...
void display(void)
{
	...
	//modelMatrix = translationMatrix * viewMatrix;
...
void uninitialize(void)
{
	...
	if(texture_sphere)
	{
		glDeleteTextures(1, &texture_sphere);
		texture_sphere = 0;
	}
