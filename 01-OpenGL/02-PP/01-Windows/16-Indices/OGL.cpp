//Date: 16/10/2022
//RTR2021 OGL PP Perspective Projection Rectangle
//Program for Indices Concept - Indexed Drawing
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("AMP OGL PP - Indexed Drawing Blue Screen Perspective Rectangle with Shaders"),
...
int initialize(void)
{
	...
	//indices are also called as elements
	//rectangleIndices are indices of 2 triangle of which our rectangle is made up of in triangle winding order
	//this technique saves common float operations using unsigned integers hence increasing performance, done in teapot program
	//this technique is known as Indexed Drawing, also in DirectX, Vulkan, Metal

	//vao for rectangle
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);	
	
	//vbo for position vertices
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(rectangleVertices), rectangleVertices, GL_STATIC_DRAW);	 	  	//putting Vertices array in vbo in VRAM; GL_STATIC_DRAW means to fill buffer data now or on runtime, for runtime use GL_DYNAMIC_DRAW
	...
	//initialize needed matrices using vmath.h
...
void display(void)
{
	...
	glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);			//there are no QUADS in PP, every geometry has to made using GL_TRIANGLES or GL_TRIANGLES_FAN or GL_TRIANGLES_STRIP
