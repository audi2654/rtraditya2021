//Date: 24/06/2022
//RTR2021 OGL XPerspective Triangle Blue Screen with Shaders
...
GLuint vbo_color;					//vertex buffer object for color
...
int main(int argc, char* argv[], char* envp[])
{
	...
	XStoreName(display, window, "AMP OGL PP Colored XPerspective Triangle with Shaders");
...
int initialize(void)
{
	...
	//vbo for position
	glGenBuffers(1, &vbo_position);							//step-A5a		here vbo returns with a memory mapped I/O address from GPU VRAM, i.e watching GPU memory(khidki) from CPU memory(khidki)
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position);			//step-A5b		binding vbo to a target point of type of GL_ARRAY_BUFFER means, this vbo addr will hold array buffer data
	glBufferData(GL_ARRAY_BUFFER, sizeof(trianglePosition), trianglePosition, GL_STATIC_DRAW);		//step-A5c		putting trianglePosition array in vbo in VRAM; GL_STATIC_DRAW means to fill buffer data now or on runtime, for runtime use GL_DYNAMIC_DRAW
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0);		//step-A5f		unbinding vertex array buffer
