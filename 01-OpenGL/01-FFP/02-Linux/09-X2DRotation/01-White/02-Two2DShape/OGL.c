//Date: 30/06/2022
//RTR2021 OGL White Perspective Projection Blue Screen - XTwo2DShapes Rotation
...
void draw(void)
{
	...
	glRotatef(angleTriangle, 0.0f, 1.0f, 0.0f);		//y axis spin rotation
	...
	//try commenting & uncommenting below both lines & see output
	//glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();

	//try commenting & uncommenting below both lines & see output
	glTranslatef(1.5f, 0.0f, -6.0f);		//for previous CTM position RESET translate call with glLoadIdentity()
	//glTranslatef(3.0f, 0.0f, 0.0f);		//previous CTM position RELATIVE translate call without glLoadIdentity()
	glRotatef(angleRectangle, 1.0f, 0.0f, 0.0f);		//x axis rolling rotation
