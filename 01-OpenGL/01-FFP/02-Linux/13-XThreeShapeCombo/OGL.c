//Date: 30/06/2022
//RTR2021 OGL XHollow Circle Triangle Rectangle Combo on Graph Paper 
...
Bool amp_gbToggleCircle = False;
Bool amp_gbToggleSquare = False;
Bool amp_gbToggleTriangle = False;
		...
		while(XPending(amp_display))								//XPending is analogous to PeekMessage() in Win32
		{
			...
			switch(amp_event.type)
			{
				...
				switch(keys[0])
				{
					...
					case 'C':
					case 'c':
						if(amp_gbToggleCircle == False)
						{
							amp_gbToggleCircle = True;
							//display();
						}
						else if (amp_gbToggleCircle == True)
						{
							amp_gbToggleCircle = False;
							//display();
						}
						break;
					...
					case 'T':
					case 't':
						if(amp_gbToggleTriangle == False)
						{
							amp_gbToggleTriangle = True;
						}
						else if (amp_gbToggleTriangle == True)
						{
							amp_gbToggleTriangle = False;
						}
						break;

					case 'S':
					case 's':
						if(amp_gbToggleSquare == False)
						{
							amp_gbToggleSquare = True;
						}
						else if (amp_gbToggleSquare == True)
						{
							amp_gbToggleSquare = False;
						}
						break;
...
void amp_draw(void)
{
	...
	if(amp_gbToggleCircle == True)
	{
		amp_Circle();
	}

	if(amp_gbToggleSquare == True)
	{
		amp_Rectangle();
	}

	if(amp_gbToggleTriangle == True)
	{
		amp_Triangle();
	}
