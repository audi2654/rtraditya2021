//Date: 01/06/2022
//RTR2021 XWindows Hello World in Center
...
int main(void)
{
	...
	//for Hello World painting
	static GC gc;									//Graphics Context, analogous to HDC in Win32
	XGCValues gcValues;
	static XFontStruct *fontStruct = NULL; 
	XColor fontColor;
	char str[] = "Hello World from XServer !!!";
	int strLength;
	int strWidth;
	int fontHeight;
	...
	windowAttributes.event_mask = ExposureMask | KeyPressMask | StructureNotifyMask;		//event mask is the member which decides which msgs our wnd should respond to & which it should not, 
	...
	//by calling XMoveWindow() after XMapWindow() we respect the position decided by WindowManager while creating the wnd, 
	//but since we want to it at a choosen position we immediately move to our desired position as soon as it is about to be visible to user
	//unlike in Windows we create the window at center position of the screen
	...
	while(1)
	{
		...
		case MapNotify:
			fontStruct = XLoadQueryFont(display, "fixed");		//System Font in Windows is called as fixed font in XWindows
			break;
			
		case KeyPress:
			keySym = XkbKeycodeToKeysym(display, event.xkey.keycode, 0, 0);				//14th Xlib API call, analogous to WM_KEYDOWN in Win32
			...
			XLookupString(&event.xkey, keys, sizeof(keys), NULL, NULL);					//analogous to WM_CHAR in Win32
		...
		/*ConfigureNotify is similar to WM_SIZE message in Win32*/	
		case ConfigureNotify:
			winWidth = event.xconfigure.width;
			winHeight = event.xconfigure.height;
			break;
		
		/*Expose is similar to WM_PAINT message in Win32*/
		case Expose:
			...
			//Extra: try increasing the font size of displayed text - need to use something else in place of  XLoadQueryFont() 
...
void toggleFullscreen(void)
{
	...
	XSendEvent(display, 
				   RootWindow(display, visualInfo.screen),
				   False,	/*3rd para means whether we want to propagate this event message to child windows or not, false is no*/
				   /*kinda analogous to GetMessage() 2nd para in Win32 where we mention of whose window msgs are to be retireved, child windows or all or current*/
				   SubstructureNotifyMask, /*SubstructureNotifyMask here is analogous to SWP_FRAMECHANGED in Win32*/
				   &event);
	//XSendEvent is similar to SendMessage
}
