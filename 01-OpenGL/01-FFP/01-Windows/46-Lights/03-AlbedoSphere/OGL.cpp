//Date: 12/05/2022
//RTR2021 OGL Gouraud Shading on Sphere Albedo
...
int initialize(void)
{
	...
	glLightfv(GL_LIGHT0, GL_SPECULAR, gfLightSpecular);
	...
	//glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);			//---put this here only
...
void uninitialize(void)
{
	...
	if (ghwnd)
	{
		DestroyWindow(ghwnd);	//we destroy here if in WinMain iRetVal = initialize(); fails, we can immediately call uninitialize() there itself
		ghwnd = NULL;
	}
